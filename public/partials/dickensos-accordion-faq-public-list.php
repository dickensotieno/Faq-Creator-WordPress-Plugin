...
?>
<!-- List Template -->
<div class="dickensos_faq_list pafa-list">
    <a id="pafa-top"></a>
            ...
            foreach( $pafa_faq_data['terms'] as $terms ) {
                if( count($pafa_faq_data['questions'][$terms->term_id]) > 0 ) {
                    ...
                    foreach( $pafa_faq_data['questions'][$terms->term_id] as $post ) {
                        setup_postdata($post);
                        ?>
                        <article id="question-<?php the_ID(); ?>" class="pafa type-dickensos_faq status-publish clearfix">
                            <h3 id="pafa-q-<?php the_ID(); ?>" class="pafa-list-q"><a name="<?php the_ID(); ?>"></a><?php the_title(); ?></h3>
                            <div class="pafa-a">
                                <?php the_content(); ?>
                                <a class="pafa-back-top" href="#pafa-top"><?php _e( 'Back To Top', 'faq-creator' ); ?></a>
                            </div>
                        </article>
                        <?php
                    }
